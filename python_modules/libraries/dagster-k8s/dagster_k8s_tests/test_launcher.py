...
def test_k8s_run_launcher():
    ...
    # Poll the job for successful completion (time out after 2 mins)
    succeeded = None
    ...
    while succeeded is None and num_iter < max_iter:
        ...
        # print('Job status: ', launched_job.status)

        pods = client.CoreV1Api().list_namespaced_pod(namespace='default')
        launched_pod_name = None
        for pod in pods.items:
            if pod.metadata.owner_references[0].name == 'dagster-job-%s' % run_id:
                launched_pod_name = pod.metadata.name

        pod = client.CoreV1Api().read_namespaced_pod(name=launched_pod_name, namespace='default')
